...
const globalFirstSolves = new Set();
...
const hasGlobalScoreEvent = async (eventType, metadataFilter) => {
    let query = supabase
        .from('score_events')
        .select('id')
        .eq('event_type', eventType)
...
app.post('/api/blackmarket/buy', authenticateToken, async (req, res) => {
    ...
    if (!item || !targetTeam)
        return res.status(400).json({ error: 'Item and target team required' });
    try {
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        if (error || !team) {
            ...
            if (createError)
                throw createError;
            const token = jwt.sign({ id: newTeam.id, name: newTeam.name, role: newTeam.role }, JWT_SECRET, { expiresIn: '48h' });
        ...
        const token = jwt.sign({ id: team.id, name: team.name, role: team.role, tokenVersion: team.token_version }, JWT_SECRET, { expiresIn: '48h' });
        res.cookie('tech_detective_token', token, {
            httpOnly: true,
            secure: process.env.NODE_ENV === 'production',
            maxAge: 48 * 60 * 60 * 1000
...
app.get('/api/auth/me', async (req, res) => {
    const token = req.cookies['tech_detective_token'] || req.headers['authorization']?.split(' ')[1];
    if (!token)
        return res.json({ error: 'Unauthorized', authenticated: false });
    jwt.verify(token, JWT_SECRET, async (err, user) => {
        if (err)
            return res.json({ error: 'Forbidden', authenticated: false });
        try {
            const { data: team, error } = await supabase
                .from('teams')
                ...
                .single();
            if (error || !team)
                return res.json({ error: 'Team not found', authenticated: false });
            res.json({ team });
        }
        catch (dbErr) {
            res.json({ team: user });
